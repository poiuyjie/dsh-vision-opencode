...
设置 → Vision 里，每个模型有一行「推理」策略，决定识图时要不要让模型思考：

| 选项 | 含义 |
|---|---|
| 默认 | 跟随供应商默认档位，识图模型正常思考 |
| 关闭 | 不思考，更快更省。只在供应商**真实声明支持关闭**时才提供（例：`hy3` 的 `off:"none"`） |
| 强制关闭 | 实验性，尽力关掉思考（自适应参数，如 `reasoning_effort:"none"`），**不保证成功**；MiMo 这类未声明 off 的模型只能选这个 |

- 有真「关闭」档的模型很少；没有时界面显示「默认 / 强制关闭」，并标注「不保证成功」。
